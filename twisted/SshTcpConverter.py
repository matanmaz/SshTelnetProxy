...
class FzSsh2TCPProtocol(protocol.Protocol):
    ...
    def connectionLost(self, reason):
        print '****FzSsh2TCPProtocol.connectionLost {}'.format(reason)
        self.tcpSide.loseConnection(reason)

    def loseConnection(self, reason):
        print '****FzSsh2TCPProtocol.loseConnection {}'.format(reason)
    ...
    def dataReceived(self, data):
        ...
        session.
        """
        print '****FzSsh2TCPProtocol.dataReceived {}'.format(toHex(data))
        if data == '\r':
            data = '\r\n'
        if data == '\x03': #^C
            self.transport.loseConnection()
    ...
    def sendMessage(self, msg):
        print '****FzSsh2TCPProtocol.sendMessage {}'.format(toHex(msg))
...
class FzTCPClient(protocol.Protocol):
    def connectionMade(self):
        self._init = True
    def sendMessage(self, msg):
        print '****FzTCPClient.sendMessage {}'.format(toHex(msg))
        self.transport.write(msg)

    def dataReceived(self, data):
        print '****FzTCPClient.dataReceived {}'.format(toHex(data))
